...
python linevul_main.py \
			...
			--block_size 512 2>&1 | tee "test_with_primevul_only.log"
...
for llm in "${llms[@]}"; do
	...
	python linevul_main.py \
		...
		--eval_data_file=$WORKDIR/data/primevul_dataset/val.csv \
		--test_data_file=$WORKDIR/data/primevul_dataset/test.csv \
		--block_size 512 2>&1 | tee "test_with_primevul_${llm}.log"
done
...
cp "$WORKDIR/linevul"/test_with_primevul_*.log "$INPUTDIR/linevul/testing_logs" 2>/dev/null || true
...
python linevul_main.py \
    ...
    --block_size 512 2>&1 | tee "test_with_reposvul_only.log"
...
for llm in "${llms[@]}"; do
  ...
  python linevul_main.py \
    ...
    --eval_data_file=$WORKDIR/data/reposvul_dataset/val.csv \
    --test_data_file=$WORKDIR/data/reposvul_dataset/test.csv \
    --block_size 512 2>&1 | tee "test_with_reposvul_${llm}.log"
done

# Ergebnisse und Logs zurueck ins Einabeverzeichnis kopieren
cp -r "$WORKDIR/linevul/saved_models_with_reposvul"* "$INPUTDIR/linevul/" 2>/dev/null || true
cp "$WORKDIR/linevul"/test_with_reposvul_*.log "$INPUTDIR/linevul/testing_logs" 2>/dev/null || true
