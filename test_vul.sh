...
for llm in "${llms[@]}"; do
	echo ">>> Testing with $llm"

	python linevul_main.py \
		--output_dir=./saved_models_with_primevul_vul_"$llm" \
		--model_type=roberta \
		--tokenizer_name=microsoft/codebert-base \
		--model_name_or_path=microsoft/codebert-base \
		--do_test \
		--train_data_file=$WORKDIR/data/primevul_with_vul_"$llm"/train_aug.csv \
		--eval_data_file=$WORKDIR/data/primevul_dataset/val.csv \
		--test_data_file=$WORKDIR/data/primevul_dataset/test.csv \
		--block_size 512 2>&1 | tee "test_with_primevul_vul_${llm}.log"
done

# Ergebnisse und Logs zurueck ins Einabeverzeichnis kopieren
cp -r "$WORKDIR/linevul/saved_models_with_primevul_vul"* "$INPUTDIR/linevul/" 2>/dev/null || true
cp "$WORKDIR/linevul"/test_with_primevul_vul_*.log "$INPUTDIR/linevul/testing_logs" 2>/dev/null || true


###################################################################################
# ########################   RepoSVul Vul Testing   ########################
...
for llm in "${llms[@]}"; do
  ...
  python linevul_main.py \
    --output_dir=./saved_models_with_reposvul_vul_"$llm" \
    --model_type=roberta \
    --tokenizer_name=microsoft/codebert-base \
    --model_name_or_path=microsoft/codebert-base \
    --do_test \
    --train_data_file=$WORKDIR/data/reposvul_with_vul_"$llm"/train_aug.csv \
    --eval_data_file=$WORKDIR/data/reposvul_dataset/val.csv \
    --test_data_file=$WORKDIR/data/reposvul_dataset/test.csv \
    --block_size 512 2>&1 | tee "test_with_reposvul_vul_${llm}.log"
done

# Ergebnisse und Logs zurueck ins Einabeverzeichnis kopieren
cp -r "$WORKDIR/linevul/saved_models_with_reposvul_vul"* "$INPUTDIR/linevul/" 2>/dev/null || true
cp "$WORKDIR/linevul"/test_with_reposvul_vul_*.log "$INPUTDIR/linevul/testing_logs" 2>/dev/null || true
